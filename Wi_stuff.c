...
{
   // haleyjd: SMMU bug fix! -- There's a static limit on the
   // number of intermission cameras, but no range-checking. This
   // function is called on all mapthings of type 5003 so it *has*
   // to be done, else this is treading in segv territory.

   if(numcameraviews < MAXCAMERAS)
   {
...
#else
  W_CacheLumpName("WIMINUS", PU_STATIC);
#endif
