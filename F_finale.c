...
boolean F_CastResponder (event_t* ev)
{
  ...
  {
    // haleyjd: check skin name for validity?
    if(castorder[castnum].type == MT_PLAYER &&
       players[displayplayer].skin->sounds[sk_pldeth] != NULL)
...
static void F_DrawPatchCol(int x, patch_t *patch, int col)
{
  ...
  if (hires)
	...
	const byte *source = (byte *) column + 3;
        ...
        byte *dest = desttop + column->topdelta * (wh);
            ...
            for(q = 0 ; q < (1 << hires) ; q ++ )
             {
                for(w = 0; w < (1 << hires) ; w ++ )
                  {
                    dest[q + w * (SCREENWIDTH << hires)] = *source;
                  }
             }
            source++;
          }
