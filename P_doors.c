      ...
      if (!--door->topcountdown)  // downcount and check
        switch(door->type)
          ...
          case genBlazeRaise:
            ...
            S_StartSoundName((mobj_t *)&door->sector->soundorg,info_sound_bdcls);
            break;

          case doorNormal:
          case genRaise:
            door->direction = plat_down; // time to go back down
            S_StartSoundName((mobj_t *)&door->sector->soundorg,info_sound_dorcls);
            break;
          ...
          case genBlazeCdO:
            door->direction = plat_up;  // time to go back up
            S_StartSoundName((mobj_t *)&door->sector->soundorg,info_sound_bdopn);
            break;
