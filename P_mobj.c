          ...
          if (!(mo->flags & MF_MISSILE) && demo_version >= 203 &&
		   ...
		   if(demo_version >= 329 && BlockingMobj && 
		      (BlockingMobj->flags2 & MF2_REFLECTIVE))
                   {
                      angle_t refangle = R_PointToAngle2(BlockingMobj->x,
                                                         BlockingMobj->y, 
							 mo->x, 
							 mo->y);
...
static void P_ZMovement (mobj_t* mo)
{
  ...
  int correct_lost_soul_bounce = 
     !demo_compatibility || (gamemission != doom2);
        ...
        if (mo->z >= mo->ceilingz - mo->height)   // bounce off ceilings
          {
		    ...
		    mo->subsector->sector->ceilingpic != sky2flatnum)
                  ...
                  // haleyjd: stop grenades from sticking to the
		  //          sky -- this is not a full fix
		  // FIXME:   some bouncers stick to the sky!
		  if (mo->flags & MF_MISSILE || mo->type == MT_GRENADE)
...
#ifdef MONSTER_FALLING_DMG
      // disabled, probably just not a good idea, but I've left
      // the code intact in case somebody likes it

      // haleyjd: falling damage -- monsters
      if((mo->flags & MF_COUNTKILL || mo->type == MT_BARREL) &&
	 !comp[comp_fallingdmg] && demo_version >= 329)
      {
      ...
      if(!comp[comp_terrain] && (mo->z - mo->momz > mo->floorz) &&
         !(mo->flags&MF_MISSILE))
      {
...
spawnit:
  ...
  if(mobj->flags2 & MF2_FLOATBOB) // haleyjd: initialize health as floatbob seed
  {
     mobj->health = P_Random(pr_floathealth);
  }
...
void P_SpawnPuff(fixed_t x, fixed_t y, fixed_t z, angle_t dir, int updown)
{
  ...
  if(drawparticles && bulletpuff_particle)
  {
...
mobj_t* P_SpawnMissile(mobj_t* source,mobj_t* dest,mobjtype_t type)
{
  ...
  // fuzzy player --  haleyjd: add total invisibility
...
mobj_t *P_SpawnPlayerMissile(mobj_t* source, mobjtype_t type)
{
  ...
  if(type == MT_GRENADE)
  {
     // nudge momz just a bit higher
     th->momz = th->momz + 2*FRACUNIT;
     th->effects = FX_GRENADE;
  }
...
//---------------------------------------------------------------------------
//
// Start new Eternity TC mobj functions
//
//---------------------------------------------------------------------------
...
// haleyjd 1/24/99:  need a new CMS function with a boolean return type
// straight from heretic source

boolean P_NewCheckMissileSpawn(mobj_t *missile)
{
   missile->x += (missile->momx >> 1);
   missile->y += (missile->momy >> 1);
   missile->z += (missile->momz >> 1);
   
   if(!P_TryMove(missile, missile->x, missile->y, false)) 
   {                                                      
...
mobj_t *P_SpawnMissileAngle(mobj_t *source, mobjtype_t type, angle_t angle, fixed_t momz)
{
   ...
   switch(type)
   {
      case MT_MNTRFX1:  // swing attack
         z = source->z+40*FRACUNIT;
         break;
      case MT_MNTRFX2:  // floor fire missile
         z = ONFLOORZ;
         break;
      default:
    ...
    return(P_NewCheckMissileSpawn(mo) ? mo : NULL);
}
...
//  this stuff was inspired and helped along a bit by hexen, but I didn't
//  just rip it out -- I know exactly what its doing and have taken liberty
...
void P_MonsterFallingDamage(mobj_t *mo)
{
        ...
        if(mom > 35*FRACUNIT && !(mo->flags2 & MF2_INVULNERABLE))
