...
// 'if' statement -- haleyjd: changed to boolean for else/elseif
boolean spec_if()
{
  int endtoken;
  svalue_t eval;
  ...
  return !!(intvalue(eval)); // boolean-ize the integer value
}

// haleyjd: elseif
boolean spec_elseif(boolean lastif)
{
  int endtoken;
  svalue_t eval;
...
static void create_variable(int start, int stop)
{
  ...
  // arrays would be horribly complicated if allowed in the HS,
  // so we'll disallow them there as well (not ideal, but practical)
  if(newvar_script == &hub_script && newvar_type == svt_array)
  {
     script_error("cannot create arrays in hub script\n");
     return;
  }
...
boolean spec_variable()
{
  ...
  else if(!strcmp(tokens[start], "array")) // haleyjd: arrays
  {
