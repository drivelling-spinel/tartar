...
void P_InitPicAnims (void)
{
  ...
  //  Init animation
#ifdef _MSC_VER // SoM 3/14/2002: VC++ aligns the struct to 24 bytes
  char        *animbuf, *animbase;
  int         numdefs;

  animbase = animbuf = W_CacheLumpName("ANIMATED",PU_STATIC);

  numdefs = (W_LumpLength(W_GetNumForName("ANIMATED")) / 23) + 1;
  animdefs = Z_Malloc(sizeof(animdef_t) * numdefs, PU_STATIC, NULL);

  for(i = 0; i < numdefs; i++)
  {
    animdefs[i].istexture = *animbuf++;
    memcpy(&animdefs[i].endname, animbuf, 9); animbuf+=9;
    memcpy(&animdefs[i].startname, animbuf, 9); animbuf+=9;
    memcpy(&animdefs[i].speed, animbuf, 4); animbuf+=4;
  }
  Z_Free (animbase);
#else
...
// killough 11/98: reformatted
//
// haleyjd 08/22/00: fixed bug found by fraggle (should be in next SMMU ver)
//
...
int P_CheckTag(line_t *line)
{
  ...
  switch (line->special)
    ...
    case 85:
        // sf: scripting
        // haleyjd: oops, 272 is the MBF reversed sky type -- fraggle
        // and I are cooperating on a move to 280 for this type.
    //case 272:   // WR
    case 273:
    case 274:   // W1
    case 275:
...
void P_CrossSpecialLine(line_t *line, int side, mobj_t *thing)
{
  ...
  switch (line->special)
    ...
    // haleyjd
    //case 272:
    case 280:  // console command (2sided)
      t_trigger = thing;
      T_RunScript(line->tag);
      break;

    case 272: // haleyjd: SMMU compatibility
       if(comp[comp_smmuline])
       {
	  t_trigger = thing;
	  T_RunScript(line->tag);
      ...
      if (speed > 0 && demo_version >= 203)
        {
          ...
          speed = (int)(((Long64) tmpusher->magnitude << 23) / (x*x+y*y+1));
        }
...
void P_InitTerrainTypes(void)  // haleyjd 07/03/99: Restore Initialization
{
   int i;
   int lump;
   int size;
   ...
   for(i = 0; TerrainTypeDefs[i].type != -1; i++)
   {
      ...
      // We only need the number. If it didn't exist, then the game would
      // bomb out trying to load and render it long before the player could
      // hit the floor and trigger this function.  Restores Ultimate DOOM to
      // functionality without an added flat wad.
...
void FloorWater(mobj_t *, fixed_t);
void FloorSludge(mobj_t *, fixed_t);
void FloorLava(mobj_t *, fixed_t);
...
void FloorWater(mobj_t *thing, fixed_t currentz)
{
   ...
   mo = P_SpawnMobj(thing->x, thing->y, currentz, MT_SPLASHBASE);
   S_StartSound(mo, sfx_gloop);
   
   mo = P_SpawnMobj(thing->x, thing->y, currentz, MT_SPLASH);
   mo->target = thing;
...
// FLOOR_SLUDGE
//
void FloorSludge(mobj_t *thing, fixed_t currentz)
{
   mobj_t *mo;
   int temp;

   mo = P_SpawnMobj(thing->x, thing->y, currentz, MT_SLUDGEBASE);
   S_StartSound(mo, sfx_muck);
   
   mo = P_SpawnMobj(thing->x, thing->y, currentz, MT_SLUDGECHUNK);
   mo->target = thing;
...
/*
   P_FindLine:  a much nicer line finding function, courtesy Hexen,
     adapted by yours truly. Probably slower, but probably easier to
