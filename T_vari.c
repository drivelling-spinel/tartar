...
svariable_t nullret = { "null", svt_int, {0} };
...
svariable_t *new_variable(script_t *script, char *name, int vtype)
{
  ...
  tagtype = script == &global_script || script==&hub_script ? PU_STATIC
                                                            : PU_LEVEL;  
...
svalue_t getvariablevalue(svariable_t *v)
{
  ...
  else if(v->type == svt_pFixed)             // haleyjd: 8-17
    {
...
{
   svariable_t *newvar;
   newvar = new_variable(&global_script, name, svt_pArray);
   newvar->value.pA = array;
