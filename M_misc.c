...
default_t defaults[] = {
  ...
  { // killough 9/8/98
    ...
    1, {0,1}, dt_number, ss_enem, wad_yes, // joel - on by default
...
*/
  ...
  
  {
    "use_startmap",
    &use_startmap, NULL,
    0, {-1, 1}, dt_number, ss_comp, wad_yes, // joel - never use startmap
  ...
  
  {
    ...
    1, {0,1}, dt_number, ss_comp, wad_yes, // joel - falling damage off by default
    "Players and enemies do not take falling damage"
  },
  
  
  // haleyjd: changed the default for this to one because
  //   it drastically affects the light level for normal levels
  //   and also, its always used in fog, where it counts
  {
    "comp_evenlight",
    &default_comp[comp_evenlight], &comp[comp_evenlight],
    1, {0,1}, dt_number, ss_comp, wad_yes,
    "Colormaps are applied unevenly to some walls"
  },

  {
    "comp_smmuline",
    &default_comp[comp_smmuline], &comp[comp_smmuline],
    0, {0,1}, dt_number, ss_comp, wad_yes,
    "Line type 272 is WR Start Script as in SMMU"
  },

  // haleyjd 02/15/02: melee z check
  {
    "comp_scratch",
    &default_comp[comp_scratch], &comp[comp_scratch],
    0, {0,1}, dt_number, ss_comp, wad_yes, // joel - off by default
    "Melee attacks aren't checked for distance along z axis"
  },
...
*/
...

  {
    "draw_particles",
    &drawparticles, NULL,
    0, {0,1}, dt_number, ss_none, wad_no, // joel - no particles
  ...
  if ((fp = fopen(name, "rb")))
    {
      ...
      fseek(fp, 0, SEEK_SET);
      // haleyjd: 
      // This doesn't allow file buffers to be implicitly freed
      // by the zone memory allocator -- bad style, and causes
      // demos read from file to cause a Z_ChangeTag error. Needs
      // to always pass in an owner.
      // *buffer = Z_Malloc(length, PU_STATIC, 0);
      *buffer = Z_Malloc(length, PU_STATIC, (void **)buffer);
