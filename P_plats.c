    ...
    case up: // plat moving up
          ...
          || plat->type == raiseToNearestAndChange)
      {
        if (!(leveltime&7) && !silentmove(plat->sector)) // sf: silentmove
          S_StartSoundName((mobj_t *)&plat->sector->soundorg, info_sound_stnmov);
      }
      ...
      if (res == crushed && (!plat->crush))
      {
        ...
        if(!silentmove(plat->sector))    // sf: silentmove
           S_StartSoundName((mobj_t *)&plat->sector->soundorg, info_sound_pstart);
      }
      else  // else handle reaching end of up stroke
      {
        if (res == pastdest) // end of stroke
        {
          ...
          if (plat->type!=toggleUpDn)
          {
            ...
            plat->status = waiting;
            if(!silentmove(plat->sector)) // sf: silentmove
               S_StartSoundName((mobj_t *)&plat->sector->soundorg, info_sound_pstop);
          }
  ...
  while ((secnum = P_FindSectorFromLineTag(line,secnum)) >= 0)
  {
    ...
    switch(type)
    {
      case raiseToNearestAndChange:
        ...
        sec->floorpic = sides[(unsigned short)(line->sidenum[0])].sector->floorpic;
