  ...
  if (!(thing->flags & MF_NOCLIP))
    {
	  ...
	  (!(thing->flags & MF_TELEPORT) &&
           thing->type != MT_MNTRFX2 &&        // haleyjd 03/27/99
	...
	return false;
      ...
      rsubsec = R_PointInSubsector(thing->x, thing->y);
      if(rsubsec && (thing->flags2 & MF2_CANTLEAVEFLOORPIC) &&
         (tmfloorpic != rsubsec->sector->floorpic))
...
static boolean PTR_ShootTraverse(intercept_t *in)
{
   ...
   if (in->isaline)
   {
        ...
        if(sidesector)
        {
          if(z < sidesector->floorheight)
          {
            if (sidesector->floorpic == skyflatnum ||
                ...
                return false;
            zdiff = FixedDiv(abs(z - sidesector->floorheight), abs(z - startz));
          ...
          else if(z > sidesector->ceilingheight)
          {
            if (sidesector->ceilingpic == skyflatnum ||
                ...
                return false;
            zdiff = FixedDiv(abs(z - sidesector->ceilingheight), abs(z - startz));
            x += FixedMul(trace.x - x, zdiff);
            y += FixedMul(trace.y - y, zdiff);
	 ...
	 if(li->backsector && 
      ...
      P_SpawnPuff(x, y, z, R_PointToAngle2(0, 0, li->dx, li->dy) - ANG90,
	          2);
