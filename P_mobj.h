...
void    P_Massacre(void);  // haleyjd 1/22/99:  kills everything
boolean P_SetMobjStateNF(mobj_t *mobj, statenum_t state); // sets state without calling action function
mobj_t  *P_SpawnMissileAngle(mobj_t *source, mobjtype_t type, angle_t angle, fixed_t momz);  // cleaner angled firing
boolean P_NewCheckMissileSpawn(mobj_t *missile); // need a different return type
