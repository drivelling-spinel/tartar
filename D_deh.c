...
// haleyjd 02/22/02: this conflicts with MSVC's definition of feof, so
// it has to go -- all feofs replaced with dehfeof as appropriate
...
char *dehfgets(char *buf, size_t n, DEHFILE *fp)
{
  ...
  if (!n || fp->size<=0)                             // If no more characters
    return NULL;
  if (n==1)
    fp->size--, *buf = *fp->inp++;
  else
...
#define DEH_MOBJINFOMAX 24 // number of ints in the mobjinfo_t structure (!) haleyjd: 24
...
struct {
   ...
   {"NOSPLASH",         0x00000002}, // no splash object
   {"NOSTRAFE",         0x00000004}, // never uses strafing logic
   {"NORESPAWN",        0x00000008}, // never respawns
   {"ALWAYSRESPAWN",    0x00000010}, // always respawns
   {"REMOVEDEAD",       0x00000020}, // removes self after death
   {"NOTHRUST",         0x00000040}, // not affected by external pushers
   {"NOCROSS",          0x00000080}, // cannot trigger special lines
   {"JUMPDOWN",         0x00000100}, // can jump down to follow player
   {"PUSHABLE",         0x00000200}, // pushable
   {"MAP07BOSS1",       0x00000400}, // triggers map07 special 1
   {"MAP07BOSS2",       0x00000800}, // triggers map07 special 2
   {"E1M8BOSS",         0x00001000}, // triggers e1m8 special
   {"E2M8BOSS",         0x00002000}, // triggers e2m8 end
   {"E3M8BOSS",         0x00004000}, // triggers e3m8 end
   {"BOSS",             0x00008000}, // mobj is a boss
   {"E4M6BOSS",         0x00010000}, // triggers e4m6 special
   {"E4M8BOSS",         0x00020000}, // triggers e4m8 special
   {"FLOATBOB",         0x00080000}, // use floatbob z movement
   {"DONTDRAW",         0x00100000}, // don't draw vissprite
   {"SHORTMRANGE",      0x00200000}, // has shorter missile range
   {"LONGMELEE",        0x00400000}, // has longer melee range
   {"RANGEHALF",        0x00800000}, // considers 1/2 distance
   {"HIGHERMPROB",      0x01000000}, // higher min. missile attack prob.
   {"CANTLEAVEFLOORPIC",0x02000000}, // can't leave floor type
   {"INVULNERABLE",     0x08000000}, // mobj is invincible
   {"DORMANT",          0x10000000}, // mobj is dormant
   {"SEEKERMISSILE",    0x20000000}, // internal, may be a tracer
   {"DEFLECTIVE",       0x40000000}, // if reflective, deflects projectiles
   {"REFLECTIVE",       0x80000000}, // mobj reflects projectiles
};
...
extern void A_UnSetInvulnerability();
extern void A_FogSpawn();
extern void A_FogMove();
extern void A_GhostOn();
extern void A_GhostOff();
extern void A_HideThing();
extern void A_UnHideThing();
extern void A_BlurOn();
extern void A_BlurOff();
extern void A_SetReflective();
extern void A_UnSetReflective();
...
deh_bexptr deh_bexptrs[] =
{
  ...
  {A_ETCBossDeath,	"A_ETCBossDeath"},
  {A_AmbSound,		"A_AmbSound"},
  {A_ColgSound,		"A_ColgSound"},
  {A_ClericAtk,		"A_ClericAtk"},
  {A_SetInvulnerability,   "A_SetInvulnerability"},
  {A_UnSetInvulnerability, "A_UnSetInvulnerability"},
  ...
  {A_Cleric2Chase,	"A_Cleric2Chase"},
  {A_Cleric2Decide,	"A_Cleric2Decide"},
  {A_Cleric2Attack,	"A_Cleric2Attack"},
  {A_ClericBreak,	"A_ClericBreak"},
  {A_DwarfDie,		"A_DwarfDie"},
  {A_CyberGuardSigh,	"A_CyberGuardSigh"},
  {A_CyberGuardWake,	"A_CyberGuardWake"},
  {A_DwarfLDOCMagic,	"A_DwarfLDOCMagic"},
  {A_DwarfFWAEMagic,	"A_DwarfFWAEMagic"},
  {A_DwarfAlterEgoChase,  "A_DwarfAlterEgoChase"},
  {A_DwarfAlterEgoAttack, "A_DwarfAlterEgoAttack"},
  {A_FireGrenade,	"A_FireGrenade"},
  {A_PhoenixTracer,	"A_PhoenixTracer"},
  {A_AxePieceFall,	"A_AxePieceFall"},
  {A_Sparkle,		"A_Sparkle"},
  {A_StartScript,	"A_StartScript"},
  {A_StartScript,	"A_RunScript"},       // haleyjd: alias for SMMU
  {A_MummyHeadScream,	"A_MummyHeadScream"}, //          compatibility
              ...
              if (!stricmp(key,"bits") && !value) // killough 10/98
                ...
                else if (!stricmp(key,"bits2") && !value) // killough 10/98
                {
                  ...
                  for (;(strval = strtok(strval,",+| \t\f\r")); strval = NULL)
                  {
                      ...
                      if (iy >= DEH_MOBJFLAG2MAX && fpout)
                        fprintf(fpout, "Could not find bit mnemonic %s\n",
                                strval);
                  }

                  // Don't worry about conversion -- simply print values
                  if (fpout) fprintf(fpout, "Bits2 = 0x%08lX = %ld \n",
                                     value, value);
                }
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
      ...
      if (!stricmp(key,deh_sfxinfo[0]))  // Offset
        /* nop */ ;  // we don't know what this is, I don't think
      else
        if (!stricmp(key,deh_sfxinfo[1]))  // Zero/One
          S_sfx[indexnum].singularity = value;
        else
          if (!stricmp(key,deh_sfxinfo[2]))  // Value
            S_sfx[indexnum].priority = value;
          else
            if (!stricmp(key,deh_sfxinfo[3]))  // Zero 1
              S_sfx[indexnum].link = (sfxinfo_t *)value;
            else
              if (!stricmp(key,deh_sfxinfo[4]))  // Zero 2
                S_sfx[indexnum].pitch = value;
              else
                if (!stricmp(key,deh_sfxinfo[5]))  // Zero 3
                  S_sfx[indexnum].volume = value;
                else
                  if (!stricmp(key,deh_sfxinfo[6]))  // Zero 4
                    S_sfx[indexnum].data = (void *) value; // killough 5/3/98: changed cast
                  else
                    if (!stricmp(key,deh_sfxinfo[7]))  // Neg. One 1
                      S_sfx[indexnum].usefulness = value;
                    else
                      if (!stricmp(key,deh_sfxinfo[8]))  // Neg. One 2
  ...
  while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
      ...
      if (!stricmp(key,deh_ammo[0]))  // Max ammo
        maxammo[indexnum] = value;
      else
        if (!stricmp(key,deh_ammo[1]))  // Per ammo
  ...
  if(extra != EXTRA_NONE) return;
   ...
   while(!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
   {
	 ...
	 fprintf(fpout,"Processing Misc item '%s'\n", key);
      ...
      if(!stricmp(key,deh_misc[0]))       // Initial Health
	 initial_health = value;
      else if(!stricmp(key,deh_misc[1]))  // Initial Bullets
	 initial_bullets = value;
      else if(!stricmp(key,deh_misc[2]))  // Max Health
	 maxhealth = value;
      else if(!stricmp(key,deh_misc[3]))  // Max Armor
	 max_armor = value;
      else if(!stricmp(key,deh_misc[4]))  // Green Armor Class
	 green_armor_class = value;
      else if(!stricmp(key,deh_misc[5]))  // Blue Armor Class
	 blue_armor_class = value;
      else if(!stricmp(key,deh_misc[6]))  // Max Soulsphere
	 max_soul = value;
      else if(!stricmp(key,deh_misc[7]))  // Soulsphere Health
	 soul_health = value;
      else if(!stricmp(key,deh_misc[8]))  // Megasphere Health
	 mega_health = value;
      else if(!stricmp(key,deh_misc[9]))  // God Mode Health
	 god_health = value;
      else if(!stricmp(key,deh_misc[10])) // IDFA Armor
	 idfa_armor = value;
      else if(!stricmp(key,deh_misc[11])) // IDFA Armor Class
	 idfa_armor_class = value;
      else if(!stricmp(key,deh_misc[12])) // IDKFA Armor
	 idkfa_armor = value;
      else if(!stricmp(key,deh_misc[13])) // IDKFA Armor Class
	 idkfa_armor_class = value;
      else if(!stricmp(key,deh_misc[14])) // BFG Cells/Shot
	 bfgcells = value;
      else if(!stricmp(key,deh_misc[15])) // Monsters Infight
...
void deh_procBexSprites(DEHFILE *fpin, FILE *fpout, char *line, extra_file_t extra)
{
   ...
   while(!dehfeof(fpin) && *inbuffer && (*inbuffer != ' '))
   {
      if(!fgets(inbuffer, sizeof(inbuffer), fpin))
	 break;
      if(*inbuffer == '#')
