	       ...
	       const char *picname)
{
   ...
   if(netgame)
   {
      sprintf(startmessage, "%s speaks to %s", 
...
void DLG_Stop(void)
{
   ...
   // set redrawdlgdone so that no scraps are left on the sides
   // of a small screen etc.
   redrawdlgdone = true;
}
...
{
   t_trigger = players[currentdialog->playernum].mo;
   T_RunScript(arg);
}
...
void DLG_Drawer(void)
{
   ...
   // draw background
   if(bgtrans)
      V_DrawPatchTL(0, 0, 0, dlgbackground, colrngs[bgcolor]);
   else
      V_DrawPatchTranslated(0, 0, 0, dlgbackground, colrngs[bgcolor], 0);

   // draw character picture
   V_DrawPatch(DIALOGPIC_X, DIALOGPIC_Y, 0, dlgpic);
...
void DLG_WriteText(void)
{
   ...
   for(; count; count--)
   {
	 ...
	 continue; // still need to get to end of message anyways
      ...
      V_DrawPatchTranslated(cx, cy, 0, v_font[c], outr, 0);
      cx += w;
   }
